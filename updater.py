...
# reconstruction_lossの更新.
    ...
    # もう少し数式に沿った書き方
    def reconstruction_loss(self, y_fake, x_real, x_fake):
        ...
        nparray_x_fake = np.array(x_fake)
        nparray_x_real = np.array(x_real)

        L4 = F.sum((np.linalg.norm(nparray_x_fake - nparray_x_real)) ** 2) / batchsize3

        loss = (1 - beta**epoch) * L3 + (beta**epoch) * L4
...
# update_coreでミニバッチに対する処理を定義
    ...
    def update_core(self):
        ...
        gen_optimizer.update(self.reconstruction_loss, gen, y_fake)           # 贋作の識別結果で生成モデルのアップデート
