...
# load models 
netG = models._netG_1(ngpu, nz, nc, ngf, n_extra_g, opt.imageSize)
netD = models._netD_1(ngpu, nz, nc, ndf, n_extra_d, opt.imageSize)
...
for iteration in range(1, opt.niter+1):
    ...
    label.data.fill_(real_label - opt.d_labelSmooth) 
    ...
    if not history.size():
        ...
        history = torch.cat([history, fake])
    ...
    output = netD(history.detach()) # add ".detach()" to avoid backprop through G
    output = lowerbound(output)
